export function use(target, trait_class) {
    /**
     ...
     -----
     Notes: In the future we could support more involved traits such as only pulling certain methods.
...
*/
function function_props (prototype)  {
    let arr : {name: string, descriptor: PropertyDescriptor}[] = [];

    // Must Pollyfill. It's not in IE11 yet.

    let properties = (Object as any).getOwnPropertyDescriptors(prototype);
